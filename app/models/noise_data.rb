class NoiseData 
  ...
  def collectionView(collectionView, didSelectItemAtIndexPath: indexPath)
    ...
    if soundExist?(sound_name)
      p 'Sound file found at documents directory'
    else
      ...
      downloadOperation.setCompletionBlockWithSuccess(lambda do |operation, responseObject| 
        playSound(sound_name)
        p 'successfully download sound file'
       end, failure: lambda do |operation, error_ptr| end)
    end
  ...
  def playSound(name)
    ...
    error_ptr = Pointer.new(:object)
    @audioPlayer = AVAudioPlayer.alloc.initWithContentOfURL(NSURL.fileURLWithPath(file_path), error: error_ptr)
